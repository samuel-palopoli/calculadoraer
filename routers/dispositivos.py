...
@router.get(
    "/unidades-consumidora/{unidade_consumidora_id}",
...
@router.get(
    "/dependencias/{dependencia_id}", response_model=DispositivoReadList
)
def listar_dispositivo(dependencia_id):
...
def atualizar_dispositivo(
    id_dispositivo: int, dispositivo_atualizado: DispositivoUpdate
):
    ...
    dispositivo.uso_diaro = dispositivo_atualizado.uso_diaro
...
def eliminar_dispositivo(dispostivo_id:int):
    dispositivo = DispositivoDB.get_or_none(DispositivoDB.id == dispostivo_id)
