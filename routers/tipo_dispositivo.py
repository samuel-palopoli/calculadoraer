...
def listar_tipo_de_dispositivo(tipo_dispositivo_id: int):
    tipo = TipoDispositivoDB.get_or_none(
        TipoDispositivoDB.id == tipo_dispositivo_id
    )
    return tipo
...
def atualizar_tipo_de_dispositivo(
    tipo_dispositivo_id: int, tipo_atualizado: TipoDispositivoUpdate):
    tipo = TipoDispositivoDB.get_or_none(
        TipoDispositivoDB.id == tipo_dispositivo_id
    )
    if tipo:
...
@router.delete(
    path='/{tipo_dispositivo_id}', response_model=TipoDispositivoRead
)
